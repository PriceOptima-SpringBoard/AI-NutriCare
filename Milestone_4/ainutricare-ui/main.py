...
def generate_structured_plan(patient_profile, clinical_data, food_candidates, dietary_preferences: Optional[DietaryPreferences] = None):
    ...
    TASK:
    Create a **7-DAY MEAL PLAN** by selecting items from the CANDIDATE FOODS list below.
    The plan must cover Day 1 through Day 7.
    ...
    5. **Variety:** Ensure different meals across the 7 days to avoid monotony.
    ...
    {{
      ...
      "medical_reasoning": "Explain the overall strategy for this week's plan."
    }}

    FINAL CHECKS:
    - Return exactly 7 days (day_1 to day_7).
    - Use EXACT nutritional values from the candidate list.
    - Calculate average_daily_nutrition correctly.
    - Return JSON ONLY.
    """
    ...
    schema = {
        ...
        "properties": {
            "weekly_plan": {
                "type": "object",
                "properties": {
                    "day_1": day_plan_schema,
                    "day_2": day_plan_schema,
                    "day_3": day_plan_schema,
                    "day_4": day_plan_schema,
                    "day_5": day_plan_schema,
                    "day_6": day_plan_schema,
                    "day_7": day_plan_schema,
                },
        ...
        "required": ["weekly_plan", "average_daily_nutrition", "medical_reasoning"],
    }
    ...
    resp = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            temperature=0.2, # Slightly higher temp for variety across days
        ),
    )
...
def round_plan(plan: Dict[str, Any], ndigits: int = 0) -> Dict[str, Any]:
    ...
    # Iterate dynamically over any key starting with "day_" or just all keys in weekly_plan
    for day_key, day_data in weekly.items():
        for meal in ["breakfast", "lunch", "dinner", "snacks"]:
            for dish in day_data.get(meal, []):
                for key in ["calories", "protein", "fat", "carbs"]:
                    val = dish.get(key)
                    if isinstance(val, (int, float)):
                        dish[key] = round(val, ndigits)

    tn = plan.get("average_daily_nutrition", {})
    for key in ["calories", "protein", "fat", "carbs"]:
        val = tn.get(key)
        if isinstance(val, (int, float)):
